...
func PostDisconnect(c *gin.Context) {
	...
	log.Printf("=== PostDisconnect called ===")
	...
	log.Printf("Target User ID: %s", req.TargetUserID)
	...
	log.Printf("Setting reaction to dislike...")
	...
	log.Printf("SUCCESS: Reaction set to dislike")

	// 2. Delete the connection
	log.Printf("Deleting connection...")
	result, err := internal.DB.Exec(ctx, `
	...
	if err != nil {
		...
		log.Printf("SUCCESS: Deleted %d connections", result.RowsAffected())
	}

	// 3. Delete the chat
	log.Printf("Deleting chat...")
	chatResult, err := internal.DB.Exec(ctx, `
		DELETE FROM chats 
	...
	if err != nil {
		...
		log.Printf("SUCCESS: Deleted %d chats", chatResult.RowsAffected())
	}

	log.Printf("=== PostDisconnect completed successfully ===")
	c.JSON(200, gin.H{
		"status":         "ok",
		"userID":         userID,
